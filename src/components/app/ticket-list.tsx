...
import { Bug, Lightbulb, Plus, User } from 'lucide-react';
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {selectedTicket.status === '배정' && <Button type="button" className="mt-6 h-11 w-full" onClick={async () => { const response = await fetch(`/api/tickets?id=${encodeURIComponent(selectedTicket.id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: '완료' }) }); if (response.ok) { setDisplayTickets((current) => current.map((ticket) => ticket.id === selectedTicket.id ? { ...ticket, status: '완료' } : ticket)); setSelectedTicket((current) => current ? { ...current, status: '완료' } : current); } }}><Plus className="mr-2 h-4 w-4" />완료 처리</Button>}
