...
export default function NextMeetingBadge({ initialValue }: { initialValue: string | null }) {
  ...
  const selectedDate = input ? new Date(`${input}:00`) : undefined;
      ...
      <DialogContent className="overflow-visible">
        ...
        <form onSubmit={save} className="mt-5 space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div ref={timeRef} className="relative space-y-2"><p className="text-sm font-medium">회의 시간</p><button type="button" onClick={() => setTimeOpen(open => !open)} className="flex h-10 w-full items-center rounded-md border border-input bg-background px-3 text-left text-sm hover:bg-accent">{input.slice(11, 16) || '시간을 선택하세요'}</button>{timeOpen && <div className="absolute left-0 top-[4.75rem] z-10 max-h-56 w-full overflow-y-auto rounded-md border bg-background p-1 shadow-lg">{Array.from({ length: 48 }, (_, index) => `${String(Math.floor(index / 2)).padStart(2, '0')}:${index % 2 ? '30' : '00'}`).map(time => <button key={time} type="button" onClick={() => { setInput(`${input.slice(0, 10)}T${time}`); setTimeOpen(false); }} className="block w-full rounded px-3 py-2 text-left text-sm hover:bg-accent">{time}</button>)}</div>}</div>
          </div>
          {error && <p className="text-sm text-destructive">{error}</p>}
          <div className="flex justify-end gap-2"><button type="button" onClick={() => setOpen(false)} className="rounded-lg border px-4 py-2 text-sm">취소</button><button type="submit" disabled={saving} className="rounded-lg bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-50">{saving ? '저장 중…' : '저장'}</button></div>
        </form>
      </DialogContent>
    </Dialog>
  </>;
