...
import { Bug, Check, Lightbulb, User } from 'lucide-react';
            ...
            return (
              <>
                <div className="flex flex-wrap items-center gap-x-8 gap-y-2 border-b border-border pb-4">
                  ...
                  <div className="flex items-center gap-3"><span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground">{selectedTicket.type === '버그' ? <Bug className="h-4 w-4" /> : <Lightbulb className="h-4 w-4" />}{selectedTicket.type || '미지정'}</span></div>
                </div>
                ...
                {selectedTicket.status === '배정' && <Button type="button" className="mt-6 h-11 w-full" onClick={async () => { const response = await fetch(`/api/tickets?id=${encodeURIComponent(selectedTicket.id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: '완료' }) }); if (response.ok) { setDisplayTickets((current) => current.map((ticket) => ticket.id === selectedTicket.id ? { ...ticket, status: '완료' } : ticket)); setSelectedTicket((current) => current ? { ...current, status: '완료' } : current); } }}><Check className="mr-2 h-4 w-4" />완료 처리</Button>}
