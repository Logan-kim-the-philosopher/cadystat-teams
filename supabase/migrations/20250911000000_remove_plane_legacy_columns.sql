-- Supabase is now the sole source of truth for organization data.
...
declare
  ...
  v_member_id uuid;
begin
  select id into v_team_id from public.teams where id::text = p_team_id for update;
  ...
  for item in select * from jsonb_array_elements(p_members) loop
    if coalesce(item->>'action', 'update') = 'delete' then
      select id into v_member_id from public.members where id::text = item->>'id';
      ...
      insert into public.members (name, title, gender, team_id, reports_to, sort_order)
      values (item->>'name', item->>'title', nullif(item->>'gender',''), v_team_id, null, coalesce((item->>'sortOrder')::integer, 0));
    else
      select id into v_member_id from public.members where id::text = item->>'id';
      if v_member_id is null or not exists (select 1 from public.members where id = v_member_id and team_id = v_team_id) then raise exception '팀원을 찾을 수 없습니다.'; end if;
      update public.members set name=item->>'name', title=item->>'title', gender=nullif(item->>'gender','') where id=v_member_id;
    end if;
  end loop;
  if nullif(p_lead_member_id, '') is null then v_lead_id := null;
  else
    select id into v_lead_id from public.members where id::text = p_lead_member_id and team_id = v_team_id;
...
alter table public.teams drop column if exists plane_project_id;
alter table public.members drop column if exists plane_work_item_id;
